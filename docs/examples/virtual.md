...
虚拟窗口会在按行高向上取整的视口行数基础上再预留一行，覆盖部分可见节点，避免同一行内的小幅滚动反复改变窗口长度。因此，即使 `virtual-overscan` 为 `0`，也不要依赖渲染行数恰好等于视口高度除以行高。
...
::: tip 快速甩动时的短暂留白
小程序的滚动位置是节流上报给逻辑层的，极快甩动时渲染会短暂落后于手势，可视区可能短暂留白。组件会在滚动事件短暂停止后主动测量并校正渲染窗口，无需通过再次触摸触发校正。适当调大 `virtual-overscan` 可以进一步减少这种留白。
:::

::: tip 何时开启
可见节点较多，尤其是使用 `default-expand-all` 展开大数据树时，可以开启虚拟渲染以减少同时渲染的行数。是否开启应结合目标设备、插槽复杂度和实际交互表现判断，没有统一的节点数阈值。
:::
